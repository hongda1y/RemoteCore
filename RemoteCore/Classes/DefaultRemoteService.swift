...
private protocol DefaultRemoteServiceRepo {
    ...
    func fetchItems<T:Codable>(with url : String,
                               ...
                               headers : HTTPHeaders?,
                               debug: Bool,
                               completion : @escaping (Result<T?,RemoteError>) -> Void)
    ...
    func deleteItem(with url : String,
                    ...
                    headers : HTTPHeaders?,
                    debug: Bool,
                    completion : @escaping (Result<DeleteResponse?,RemoteError>) -> Void)
...
public class DefaultRemoteService {
    private let remote = RemoteCore()
}


extension DefaultRemoteService : DefaultRemoteServiceRepo {
    ...
    public func fetchItems<T>(with url: String,
                       ...
                       headers: HTTPHeaders?,
                       debug: Bool = false,
                       completion: @escaping (Result<T?, RemoteError>) -> Void) where T : Decodable, T : Encodable {
 
        let configure : RemoteConfigure<T> = .init(url: url,
                                                   method: .get,
                                                   params: params,
                                                   headers: headers,
                                                   debug: debug) {
        ...
        remote.request(configuration: configure)
    ...
    public func addItem<T>(_ item: T,
        ...
        guard let headers = headers else {
            completion(.failure(.csError(.missing_header)))
            return
        }
        ...
        guard let jData = jsonData else {
            completion(.failure(.csError(.json_response_error)))
            return
        }
        ...
        let configure : RemoteURLRequestConfigure<T> = .init(urlRequest: urlRequest, debug: debug) {
            completion($0)
        }
    ...
    public func deleteItem(with url: String,
        ...
        let configure : RemoteConfigure<DeleteResponse> = .init(url: url,
                                                   method: .delete,
                                                   params: parameters,
                                                   headers: headers,
                                                   debug: debug) {
    ...
    public func uploadImage<T>(of type: T.Type,
        ...
        remote.uploadFile(configuration: configure)
